...
const US_coords = { lat: 37.7749, lng: -122.4194 };
...
class MapContainer extends Component {
    ...
    //Runs when google maps library is ready \\
    ...
    apiHasLoaded = (map, mapsApi) => {
        this.setState({
            ...
            map,
            mapsApi,
            usaLatLng: new mapsApi.LatLng(US_coords.lat, US_coords.lng),
    ...
    render() {
        ...
        return ( <
            ...
            <
            h1 className = "w-100 fw-md" > Find Some Ice - Creams! < /h1>{" "} { /* Constraints section */ } { " " } <
                    ...
                    div > { " " } {
                        constraints.map((constraint, key) => {
                            ...
                            return ( <
                                ...
                                />{" "} <
                                AutoCompleteMap autoCompleteService = { autoCompleteService }
            ...
            defaultZoom = { 11 }
            defaultCenter = {
                { lat: US_coords.lat, lng: US_coords.lng } }
            yesIWantToUseGoogleMapApiInternals = { true }
            onGoogleApiLoaded = {
                ({ map, maps }) => this.apiHasLoaded(map, maps) } // "maps" is the mapApi. Bad naming but that's their library.
            >
            { /* Pin markers on the Map*/ } { " " } {
                markers.map((marker, key) => {
                    const { name, lat, lng } = marker;
                    return <MapMarker key = { key }
                    name = { name }
                    lat = { lat }
                    lng = { lng }
                    />;
